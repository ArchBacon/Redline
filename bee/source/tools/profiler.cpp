...
BEE_DISABLE_WARNING_MISSING_FIELD_INITIALIZERS
...
ProfilerSection::ProfilerSection(std::string name) : m_name(std::move(name)) { Engine.Profiler().BeginSection(m_name); }

ProfilerSection::~ProfilerSection() { Engine.Profiler().EndSection(m_name); }

ScopeProfiler::ScopeProfiler(std::string name) : m_name(std::move(name)), m_start(std::chrono::high_resolution_clock::now()) {}
...
void Profiler::BeginSection(const std::string& name) { m_times[name].Start = std::chrono::high_resolution_clock::now(); }
