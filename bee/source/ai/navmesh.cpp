...
Navmesh::Navmesh(const PolygonList& walkableAreas, const PolygonList& obstacles, float agentRadius)
{
    ...
    m_polygons = geometry2d::TriangulatePolygons(boundaries);

    // Create a dual graph for navigation
    m_graph = EuclideanGraph::CreateDualGraph(m_polygons);
}
...
void FunnelAlgorithmStepLeft(const glm::vec2& newPoint,
                             std::deque<glm::vec2>& leftFunnel,
                             std::deque<glm::vec2>& rightFunnel,
                             Path& path)
{
...
void FunnelAlgorithmStepRight(const glm::vec2& newPoint,
                              std::deque<glm::vec2>& leftFunnel,
                              std::deque<glm::vec2>& rightFunnel,
                              Path& path)
{
