...
/// <summary>
/// Computes and returns the first intersection point of a ray and a disk, if such a point exists.
/// </summary>
bool RaycastCheckDisk(const vec2& rayOrigin,
                      const vec2& rayDirection,
                      const vec2& diskPos,
                      float diskRadius,
                      RaycastResult& result)
{
    result.m_rayDistance = 0.f;

    // Ray equation: P + V * t
    // Circle: center C, radius r
    // Solve for t: dist(P + V * t, C) = r
    // => ||P + V * t - C||^2 = r^2
    // => (Px + Vx * t - Cx)^2 + (Py + Vy * t - Cy)^2
    // => (Vx*Vx)t^2 + Vx(Px-Cx)t + Cx*Cx + [same for y] - r^2 = 0
    // => (V.V)t^2 + (V.P - V.C)t + C.C - r^2 = 0
    // Solve this quadratic equation:
    float a = dot(rayDirection, rayDirection);
    float b = 2 * (dot(rayDirection, rayOrigin) - dot(rayDirection, diskPos));
    float c = dot(diskPos, diskPos) + dot(rayOrigin, rayOrigin) - 2 * dot(rayOrigin, diskPos) - diskRadius * diskRadius;
    float D = b * b - 4 * a * c;
    if (D < 0) return false;

    float sqrtD = sqrt(D), inv2a = 1 / (2 * a);
    float t1 = (-b - sqrtD) * inv2a;
    float t2 = (-b + sqrtD) * inv2a;

    if (t1 > 0)
    {
        result.m_rayDistance = t1;
        result.m_hitPosition = rayOrigin + rayDirection * t1;
        result.m_normal = normalize(result.m_hitPosition - diskPos);
        return true;
    }

    if (t2 > 0)
    {
        result.m_rayDistance = t2;
        result.m_hitPosition = rayOrigin + rayDirection * t2;
        result.m_normal = normalize(result.m_hitPosition - diskPos);
        return true;
    }
...
/// <summary>
/// Computes and returns the first intersection point of a ray and a line segment, if such a point exists.
/// </summary>
bool RaycastCheckSegment(const vec2& rayOrigin,
                         const vec2& rayDirection,
                         const vec2& segment1,
                         const vec2& segment2,
                         RaycastResult& result)
{
    ...
    // Ray-segment intersection, based on this solution:
    // https://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect/565282
    const vec2& segOrigin = segment1;
    const vec2& segDirection = segment2 - segment1;

    // check if the lines are not almost parallel
    float denom = PerpDot(rayDirection, segDirection);
    if (abs(denom) < 0.0001f) return false;

    // check if the intersection point is not behind the ray origin
    float t = PerpDot(segOrigin - rayOrigin, segDirection) / denom;
    if (t <= 0) return false;

    // check if the intersection point is on the segment
    float u = PerpDot(rayOrigin - segOrigin, rayDirection) / -denom;
    if (u <= 0 || u >= 1) return false;

    result.m_hitPosition = rayOrigin + t * rayDirection;
    result.m_rayDistance = t;
    result.m_normal = normalize(GetPerpendicularVector(segment2 - segment1));
    return true;
}

/// <summary>
/// Computes and returns the first intersection point of a ray and a capsule, if such a point exists.
/// </summary>
bool RaycastCheckCapsule(const vec2& rayOrigin,
                         const vec2& rayDirection,
                         const vec2& capsulePos1,
                         const vec2& capsulePos2,
                         float capsuleRadius,
                         RaycastResult& result)
{
    result.m_rayDistance = 0.f;

    RaycastResult subResult;

    // check the first disk
    if (RaycastCheckDisk(rayOrigin, rayDirection, capsulePos1, capsuleRadius, subResult) && subResult < result)
        subResult = result;

    // check the second disk
    if (RaycastCheckDisk(rayOrigin, rayDirection, capsulePos2, capsuleRadius, subResult) && subResult < result)
        subResult = result;

    // check the side segments
    const vec2& dir = normalize(capsulePos2 - capsulePos1);
    const vec2& side = GetPerpendicularVector(dir) * capsuleRadius;
    if (RaycastCheckSegment(rayOrigin, rayDirection, capsulePos1 + side, capsulePos2 + side, subResult) && subResult < result)
        subResult = result;
...
/// <summary>
/// Computes and returns the first intersection point of a ray and a polygon, if such a point exists.
/// </summary>
bool RaycastCheckPolygon(const vec2& rayOrigin, const vec2& rayDirection, const Polygon& polygonPoints, RaycastResult& result)
{
    result.m_rayDistance = 0.f;

    const size_t n = polygonPoints.size();
    for (size_t i = 0; i < n; ++i)
    {
        RaycastResult subresult;
        if (RaycastCheckSegment(rayOrigin, rayDirection, polygonPoints[i], polygonPoints[(i + 1) % n], subresult) &&
            subresult < result)
        {
            result = subresult;
        }
                             ...
                             PolygonCollider* polygon2)
{
    ...
    const vec2& v1 = body1.m_linearVelocity + body1.m_angularVelocity * perp1;
    const vec2& v2 = body2.m_linearVelocity + body2.m_angularVelocity * perp2;
...
void World::UpdateCollisionDetection()
{
    ...
    for (const auto& [entity1, body1, disk1] : view_disk.each())
    {
        ...
        for (const auto& [entity2, body2, disk2] : view_disk.each())
        {
            ...
            if (CollisionCheckDiskDisk(body1.m_position, disk1.GetRadius(), body2.m_position, disk2.GetRadius(), collision))
            {
        ...
        for (const auto& [entity2, body2, capsule2] : view_capsule.each())
        {
            ...
            const vec2& pos2A = body2.m_position - halfOffset2;
            const vec2& pos2B = body2.m_position + halfOffset2;
            CollisionData collision;
            if (CollisionCheckDiskCapsule(body1.m_position, disk1.GetRadius(), pos2A, pos2B, capsule2.GetRadius(), collision))
            {
                ResolveCollision(collision, body1, body2);
                RegisterCollision(collision, entity1, body1, entity2, body2);
            }
        ...
        for (const auto& [entity2, body2, polygon2] : view_polygon.each())
        {
            ...
            CollisionData collision;
            if (CollisionCheckDiskPolygon(body1.m_position, disk1.GetRadius(), polygon2.GetWorldPoints(), collision))
            {
    ...
    for (const auto& [entity1, body1, capsule1] : view_capsule.each())
    {
        ...
        const vec2& pos1A = body1.m_position - halfOffset1;
        const vec2& pos1B = body1.m_position + halfOffset1;
...
void World::Update(float dt)
{
    ...
    if ((Engine.DebugRenderer().GetCategoryFlags() & bee::DebugCategory::Physics) != 0)
    {
        ...
        for (const auto& [entity, body, disk] : view_disk.each())
        {
            ...
            Engine.DebugRenderer().AddLine(
                ...
                vec3(body.m_position, 0.01f),
                vec3(body.m_position + RotateCounterClockwise(vec2(disk.GetRadius(), 0.f), body.m_angle), 0.01f),
                color);
        }
        ...
        for (const auto& [entity, body, polygon] : view_polygon.each())
        {
            ...
            for (size_t i = 0; i < n; ++i)
            {
                Engine.DebugRenderer().AddCircle(
                    bee::DebugCategory::Physics,
                    vec3(body.m_position + RotateCounterClockwise(polygon.GetLocalPoints()[i], body.m_angle), 0.01f),
                    0.1f,
                    vec3(0, 0, 1),
                    color);
                Engine.DebugRenderer().AddLine(
                    bee::DebugCategory::Physics,
                    vec3(body.m_position + RotateCounterClockwise(polygon.GetLocalPoints()[i], body.m_angle), 0.01f),
                    vec3(body.m_position + RotateCounterClockwise(polygon.GetLocalPoints()[(i + 1) % n], body.m_angle), 0.01f),
                    color);
            }
        }
        ...
        for (const auto& [entity, body, capsule] : view_capsule.each())
        {
            ...
            const vec2& p1 = body.m_position - halfOffset;
            const vec2& p2 = body.m_position + halfOffset;
    ...
    // Make the visualization smoothly interpolate between two physics frames.
    float alpha = m_timeSinceLastFrame / m_fixedDeltaTime;

    for (const auto& [entity, body, transform] : Engine.ECS().Registry.view<Body, Transform>().each())
    {
        if (body.GetType() != Body::Type::Static)
        {
            const vec2& pos = mix(body.m_previousPosition, body.m_position, alpha);
            transform.SetTranslation(vec3(pos.x, pos.y, transform.GetTranslation().z));

            float ang = mix(body.m_previousAngle, body.m_angle, alpha);
            transform.SetRotation(angleAxis(ang, vec3(0, 0, 1)));
        }
    }
}

std::optional<RaycastResult> World::RaycastGetNearest(const vec2& origin, const vec2& direction)
{
    std::optional<RaycastResult> result{};
    RaycastResult subresult;

    for (const auto& [entity, body, disk] : Engine.ECS().Registry.view<Body, DiskCollider>().each())
    {
        if (RaycastCheckDisk(origin, direction, body.m_position, disk.GetRadius(), subresult))
        {
            if (!result.has_value() || subresult.m_rayDistance < result.value().m_rayDistance)
            {
                subresult.m_hitEntity = entity;
                result = subresult;
            }
        }
    }

    for (const auto& [entity, body, capsule] : Engine.ECS().Registry.view<Body, CapsuleCollider>().each())
    {
        const vec2& halfOffset = RotateCounterClockwise(vec2(0, 0.5f * capsule.GetHeight()), body.m_angle);
        const vec2& posA = body.m_position - halfOffset;
        const vec2& posB = body.m_position + halfOffset;
        if (RaycastCheckCapsule(origin, direction, posA, posB, capsule.GetRadius(), subresult))
        {
            if (!result.has_value() || subresult.m_rayDistance < result.value().m_rayDistance)
            {
                subresult.m_hitEntity = entity;
    ...
    for (const auto& [entity, body, polygon] : Engine.ECS().Registry.view<Body, PolygonCollider>().each())
    {
        if (RaycastCheckPolygon(origin, direction, polygon.GetWorldPoints(), subresult))
        {
            if (!result.has_value() || subresult.m_rayDistance < result.value().m_rayDistance)
            {
                subresult.m_hitEntity = entity;
...
std::vector<RaycastResult> World::RaycastGetAll(const glm::vec2& origin, const glm::vec2& direction)
{
...
std::vector<bee::Entity> World::GetAllObjectsInRange(const glm::vec2& pos, float radius)
{
    std::vector<bee::Entity> results;
    CollisionData subresult;

    for (const auto& [entity, body, disk] : Engine.ECS().Registry.view<Body, DiskCollider>().each())
    {
        if (CollisionCheckDiskDisk(pos, radius, body.m_position, disk.GetRadius(), subresult))
        {
    ...
    for (const auto& [entity, body, capsule] : Engine.ECS().Registry.view<Body, CapsuleCollider>().each())
    {
        ...
        const vec2& posA = body.m_position - halfOffset;
        const vec2& posB = body.m_position + halfOffset;
        if (CollisionCheckDiskCapsule(pos, radius, posA, posB, capsule.GetRadius(), subresult))
        {
            results.push_back(entity);
        }
    }
    ...
    for (const auto& [entity, body, polygon] : Engine.ECS().Registry.view<Body, PolygonCollider>().each())
    {
        if (CollisionCheckDiskPolygon(pos, radius, polygon.GetWorldPoints(), subresult))
        {
            results.push_back(entity);
        }
    }
