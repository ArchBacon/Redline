...
// disable this linting warning because of Jolt's implementation
...
JPH::TempAllocatorImpl* JoltSystem::m_tempAllocator = nullptr;
JPH::JobSystem* JoltSystem::m_jobSystem = nullptr;
...
bool JoltSystem::AddPhysicsBody(Entity entity,
                                ...
                                bool isStatic)
{
    ...
    // If Jolt could not create the body (most likely because the max #bodies has been reached), stop here
    if (bodyID == BodyID()) return false;

    // Otherwise, create an ECS component
...
void JoltSystem::Update(float dt)
{
    ...
    const auto& bodyView = Engine.ECS().Registry.view<JoltBody, Transform>();

    // execute physics frames?
    m_hasExecutedFrame = false;
    while (m_timeSinceLastUpdate >= m_fixedDeltaTime)
    {
        for (const auto& [entity, joltBody, transform] : bodyView.each())
        {
            BodyID bid(joltBody.m_bodyID);
            joltBody.m_previousPosition = ToGlm(bodyInterface.GetCenterOfMassPosition(bid));
            joltBody.m_previousRotation = ToGlm(bodyInterface.GetRotation(bid));
        }

        m_physicsSystem->Update(m_fixedDeltaTime, 5, m_tempAllocator, m_jobSystem);
        m_timeSinceLastUpdate -= m_fixedDeltaTime;
        m_hasExecutedFrame = true;
    }

    // Sync Jolt shapes with Bee: make the transforms smoothly interpolate between two physics frames.
    float alpha = m_timeSinceLastUpdate / m_fixedDeltaTime;
    for (const auto& [entity, joltBody, transform] : bodyView.each())
    {
        BodyID bid(joltBody.m_bodyID);
        if (bodyInterface.IsAdded(bid) && bodyInterface.GetObjectLayer(bid) == JoltLayers::MOVING)
        {
            const auto& pos = ToGlm(bodyInterface.GetCenterOfMassPosition(bid));
            const auto& rot = ToGlm(bodyInterface.GetRotation(bid));
            transform.SetTranslation(mix(joltBody.m_previousPosition, pos, alpha));
            transform.SetRotation(mix(joltBody.m_previousRotation, rot, alpha));
        }
    }
}
...
void JoltSystem::DebugDrawShape(const Shape* shape, const JoltBody& body, const Transform& transform, const vec3& worldOffset)
{
    ...
    {
        const auto* ocom = reinterpret_cast<const JPH::OffsetCenterOfMassShape*>(shape);
    ...
    if (shape->GetSubType() == EShapeSubType::Box)
    {
        ...
        const auto* box = reinterpret_cast<const JPH::BoxShape*>(shape);
    ...
    {
        vec4 shapeColor(1, 1, 1, 1);
        const auto* sphere = reinterpret_cast<const JPH::SphereShape*>(shape);
    ...
    else if (shape->GetSubType() == EShapeSubType::ConvexHull)
    {
        ...
        const auto* convexHull = reinterpret_cast<const JPH::ConvexHullShape*>(shape);
