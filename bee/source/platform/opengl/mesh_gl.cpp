...
Mesh::Mesh(const Model& model, size_t meshIndex, size_t primitiveIndex) : Resource(ResourceType::Mesh)
{
    for (auto& v : m_vbo) v = 0;
    ...
    m_path = GetPath(model, meshIndex, primitiveIndex);
    ...
    assert(mesh.primitives.size() > primitiveIndex);
    ...
    for (const auto& attribute : primitive.attributes)
    {
        ...
        if (attribute.first == "POSITION")
        {
            ...
            glCreateBuffers(1, &m_vbo[POSITION_LOCATION]);  // NOLINT(readability-container-data-pointer)
    ...
    if (m_vbo[TANGENT_LOCATION] == 0) ComputeTangents(model, meshIndex, primitiveIndex);
...
void Mesh::ComputeTangents(const Model& model, size_t meshIndex, size_t primitiveIndex)
{
    const auto& document = model.GetDocument();
    auto mesh = document.meshes[meshIndex];
    ...
    if (typeSize == 2)
    {
        ...
        indices.resize(accessor.count);
        for (size_t i = 0; i < temp.size(); i++) indices[i] = temp[i];
    }
...
std::string Mesh::GetPath(const Model& model, size_t meshIndex, size_t primitiveIndex)
{
    const auto& mesh = model.GetDocument().meshes[meshIndex];
    return model.GetPath() + " | Mesh-" + to_string(meshIndex) + ": " + mesh.name + " (Primitive " + to_string(primitiveIndex) +
           ")";
}

std::vector<vec4> Mesh::ComputeTangents(const std::vector<vec3>& positions,
