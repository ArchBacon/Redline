...
int PlatformFormat(Image::Format format)
{
    switch (format)
    {
        case Image::Format::kSrgb:
            return GL_SRGB8_ALPHA8;
        case Image::Format::kUnorm:
        default:
            return GL_RGBA8;
    }
}

Image::Image(const Model& model, int index, Format format) : Resource(ResourceType::Image), m_format(format)
{
    ...
    if (image.uri.empty())
    {
        if (image.bufferView >= 0)
        {
            ...
            {
                Log::Error("Image could not be loaded from a PNG file. Image:{} URI:{}",
                           GetPath(model, index, format),
                           image.uri);
            }
        }
    ...
    else
    {
        ...
        {
            Log::Error("Image could not be loaded from a file. Image:{} URI:{}", GetPath(model, index, format), image.uri);
        }
    }
...
Image::Image(FileIO::Directory directory, const std::string& path, Format format)
    : Resource(ResourceType::Image), m_format(format)
{
...
string Image::GetPath(const Model& model, int index, Format format)
{
    const auto& image = model.GetDocument().images[index];
    return model.GetPath() + " | Texture-" + to_string(index) + ": " + image.name + "_" + FormatToString(format);
}
...
void Image::CreateGLTextureWithData(unsigned char* data, int width, int height, int channels, bool genMipMaps)
{
    ...
    {
        case 1:
            // Leaving this as is, on other platforms
            // is not supported atm
            format = GL_R8;
            usage = GL_RED;
            break;
        case 4:
            format = PlatformFormat(m_format);
            usage = GL_RGBA;
            break;
        default:
            assert(false);
