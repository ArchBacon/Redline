...
public:
    ...
    void SetValue(const glm::mat4& mtx, bool transpose = false) const;

    /// Set the value. Parameter should be Texture.
    void SetValue(const Image& image) const;

protected:
    /// The shader creates a parameter.
    ShaderParameter(Shader* shader, std::string name, unsigned int type, int location, int sampler = -1)
        : m_shader(shader), m_name(std::move(name)), m_type(type), m_location(location), m_sampler(sampler)
    {
    }
...
public:
    ...
    void SetAttributePointer(int size, unsigned int type, unsigned char normalized, int stride, const void* pointer) const;
...
protected:
    ...
    ShaderAttribute(Shader* shader, std::string name, unsigned int type, int location)
        : _shader(shader), _name(std::move(name)), _type(type), _location(location)
    {
    }
...
public:
    ...
    Shader(FileIO::Directory directory,
...
private:
    ...
    static std::string GetPath(FileIO::Directory,
                               const std::string& vertexFilename,
                               const std::string& geometryFilename,
                               const std::string& fragmentFilename);
