...
/// <summary>
/// Serializers a class instance into a JSON object, provided the class has a BEE_VISITABLE_STRUCT macro defined for its
/// members.
/// </summary>
...
/// <summary>
/// Deserializers a JSON object into a class instance, provided the class has a BEE_VISITABLE_STRUCT macro defined for its
/// members.
/// </summary>
...
public:
    ...
    inline static bool Deserialize(T& result, bee::FileIO::Directory directory, const std::string& name)
    {
        ...
        if (!contents.empty())
        {
            JsonDeserializer jd(nlohmann::json::parse(contents));
...
private:
    nlohmann::json m_json;
    JsonDeserializer(nlohmann::json j) : m_json(std::move(j)) {}
