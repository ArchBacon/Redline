...
struct JoltBody
{
    ...
    glm::quat m_previousRotation{1, 0, 0, 0};
};
...
public:
    ...
    /// <summary>
    /// Tries to add a physics body to Jolt, and returns whether or not that succeeded.
    /// </summary>
    /// <returns>true if the body was successfully added to Jolt, false otherwise (e.g. if the maximum number of bodies has been
    /// reached).</returns>
    static bool AddPhysicsBody(bee::Entity entity,
    ...
    static void SetBodyPosition(const JoltBody& body, const glm::vec3& pos);
    static void SetBodyRotation(const JoltBody& body, const glm::quat& rot);
    static void SetBodyVelocity(const JoltBody& body, const glm::vec3& vel);
