...
struct CollisionData
{
    ...
    /// </summary>
    bee::Entity entity1{entt::null};

    /// <summary>
    /// The ID of the second entity involved in the collision.
    /// </summary>
    bee::Entity entity2{entt::null};
...
struct IComputeMassData
{
    ...
    IComputeMassData() = default;
    virtual ~IComputeMassData() = default;
    IComputeMassData(const IComputeMassData&) = default;
    IComputeMassData& operator=(const IComputeMassData&) = default;
    IComputeMassData(IComputeMassData&&) = default;
    IComputeMassData& operator=(IComputeMassData&&) = default;
};
...
public:
    ...
    inline void SetPosition(const glm::vec2& pos, bool interpolate = false)
    {
        m_position = pos;
        if (!interpolate) m_previousPosition = pos;
    }
    ...
    inline void SetRotation(float angle, bool interpolate = false)
    {
        m_angle = angle;
        if (!interpolate) m_previousAngle = angle;
    }
...
private:
    ...
    glm::vec2 m_previousPosition{0.f, 0.f};
