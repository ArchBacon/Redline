...
flags_to_remove = ["-xc++", "-ffp-model=precise", "-O0", "-fno-fast-math", "-fjmc"]
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="x64",
        choices=["x64"],
        help="Specify the platform (x64)",
    )
    parser.add_argument(
        "--target",
        default="bee.vcxproj",
        help=".vcxproj file that should be used for the build.",
    )
    parser.add_argument(
        "--target-path",
        default=None,
        help="absolute or relative path to target vcxproj",
    )
...
if __name__ == "__main__":
    ...
    create_compile_commands_with_msbuild(args.target, args.platform, args.target_path)
